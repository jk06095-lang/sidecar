...
export default function News() {
    ...
    const handleResearch = useCallback(async (queryOverride?: string) => {
        ...
        setIsResearchExpanded(true); // auto-expand on research
    ...
    return (
        <div className="flex h-full bg-slate-950 overflow-hidden font-mono">
            ...
            <div className={cn(
                "shrink-0 border-l border-slate-800/50 bg-slate-900/40 flex overflow-hidden research-sidebar-transition",
                isResearchExpanded ? "w-[620px] flex-row" : "w-[380px] flex-col"
            )}>
                {/* ── Scrapbook Section ── */}
                <div className={cn(
                    "flex flex-col overflow-hidden scrapbook-transition",
                    isResearchExpanded
                        ? "w-[220px] shrink-0 scrapbook-card-mode m-2 mr-0"
                        : "w-full flex-1"
                )}>
                    {/* Scrapbook header */}
                    <div className={cn(
                        "shrink-0 border-b border-slate-800/50",
                        isResearchExpanded ? "p-3" : "p-4"
                    )}>
                        <div className="flex items-center gap-2">
                            <div className={cn(
                                "rounded-lg bg-amber-500/15 border border-amber-500/30 flex items-center justify-center",
                                isResearchExpanded ? "w-6 h-6" : "w-8 h-8"
                            )}>
                                <BookmarkCheck size={isResearchExpanded ? 12 : 16} className="text-amber-400" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h2 className={cn(
                                    "font-bold text-slate-100 truncate",
                                    isResearchExpanded ? "text-xs" : "text-sm"
                                )}>스크랩 북</h2>
                            ...
                            {isResearchExpanded && (
                                <button
                                    ...
                                    className="p-1 rounded-md text-slate-500 hover:text-amber-400 hover:bg-amber-500/10 transition-all"
                                    title="스크랩북 확장"
                                >
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0">
                        {scraps.length === 0 ? (
                            <div className={cn(
                                ...
                                isResearchExpanded ? "h-32 px-3" : "h-48 px-6"
                            )}>
                                <Bookmark size={isResearchExpanded ? 18 : 24} className="text-slate-700 mb-3" />
                            ...
                            <div className={cn("space-y-1", isResearchExpanded ? "p-1.5" : "p-2")}>
                                {scraps.map(scrap => (
                                    <div
                                        key={scrap.id}
                                        onClick={() => setSelectedScrap(selectedScrap?.id === scrap.id ? null : scrap)}
                                        className={cn(
                                            "group rounded-lg cursor-pointer transition-all border",
                                            isResearchExpanded ? "p-2" : "p-3",
                                    ...
                                    >
                                        <div className="flex items-start gap-2 mb-1">
                                            <FileText size={isResearchExpanded ? 10 : 12} className="text-amber-400 mt-0.5 shrink-0" />
                                            <h4 className={cn(
                                                "font-medium text-slate-200 leading-snug flex-1",
                                                isResearchExpanded ? "text-[10px] line-clamp-1" : "text-xs line-clamp-2"
                                            )}>{scrap.title}</h4>
                                            <button
                ...
                {/* ── Research Panel ── */}
                <div className={cn(
                    "flex flex-col overflow-hidden research-expand-transition",
                    isResearchExpanded
                        ? "flex-1 border-l border-slate-800/50 research-expanded-glow"
                        : "shrink-0 border-t border-slate-800/50 w-full"
                )}
                >
                    <div className={cn(
                        "bg-slate-900/60 flex flex-col",
                        isResearchExpanded ? "flex-1 overflow-hidden" : ""
                    )}>
                        <div className={cn("shrink-0", isResearchExpanded ? "p-4 pb-3" : "p-3")}>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <button
                                    ...
                                    title={isResearchExpanded ? '패널 축소' : '패널 확대'}
                                >
                                    {isResearchExpanded ? <Minimize2 size={13} /> : <Maximize2 size={13} />}
                                </button>
                            </div>
                            <div className="flex gap-2">
                                <input
                                    ...
                                    onKeyDown={e => {
                                        ...
                                        if (e.key === 'Escape' && isResearchExpanded) setIsResearchExpanded(false);
                                    }}
                        ...
                        {/* Research results — scrollable in expanded mode */}
                        <div className={cn(
                            "px-3 pb-3",
                            isResearchExpanded ? "flex-1 overflow-y-auto custom-scrollbar" : ""
                        )}>
                            {isResearching && (
                                <div className="mt-1 p-4 rounded-lg bg-violet-950/20 border border-violet-800/20 flex items-center gap-3">
                            ...
                            {researchError && (
                                <div className="mt-1 p-3 rounded-lg bg-rose-950/20 border border-rose-800/20">
                            ...
                            {researchResult && !isResearching && (
                                <div className={cn(
                                    "mt-1 space-y-2",
                                    !isResearchExpanded && "max-h-64 overflow-y-auto custom-scrollbar"
                                )}>
