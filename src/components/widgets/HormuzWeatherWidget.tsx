...
export default function HormuzWeatherWidget() {
    const [data, setData] = useState<{ time: string, wave: number, current: number }[]>([]);
    ...
    useEffect(() => {
        const fetchWeather = async () => {
            try {
                // Fetch last 24 hours of marine weather at Strait of Hormuz (Lat 26.56, Lon 56.25)
                const response = await fetch(
                    'https://marine-api.open-meteo.com/v1/marine?latitude=26.56&longitude=56.25&hourly=wave_height,ocean_current_velocity&timezone=auto&past_days=1&forecast_days=1'
                );
                if (!response.ok) throw new Error('API Error');
                const json = await response.json();

                const hourly: WeatherData = json.hourly;
                const formattedData = hourly.time.map((t, i) => ({
                    time: new Date(t).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                    wave: hourly.wave_height[i],
                    current: hourly.ocean_current_velocity[i],
                })).filter((_, i) => i % 2 === 0).slice(0, 24); // Every 2 hours for 24 periods
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-cyan-500 gap-3 py-10">
                ...
                <span className="text-xs font-mono">Fetching Live Telemetry [Hormuz]...</span>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-rose-500 gap-2 py-10">
                ...
                <span className="text-xs font-mono">Telemetry Link Failure</span>
            </div>
        );
    }

    const currentWave = data.length > 0 ? data[12]?.wave || data[0].wave : 0;
    ...
    return (
        <div className="flex flex-col h-full bg-slate-900/40 rounded-lg p-4 border border-slate-700/30 relative overflow-hidden group">
            {/* Background Grid Pattern (Foundry Style) */}
            ...
            <div className="flex justify-between items-start z-10 mb-4">
                <div>
                    ...
                    <h4 className="text-sm font-semibold text-slate-200">Strait of Hormuz Telemetry</h4>
                    <p className="text-xs text-slate-500 mt-0.5 font-mono">Lat: 26.56°N / Lon: 56.25°E</p>
                </div>
            ...
            <div className="flex-1 w-full min-h-[160px] z-10">
