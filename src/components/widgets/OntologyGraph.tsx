...
export default function OntologyGraph({ data }: OntologyGraphProps) {
                ...
                data.forEach(item => {
                    if (!syncedNodes.find((n: Node) => n.id === item.id)) {
                        syncedNodes.push({
                            ...
                            type: item.type === 'factor' ? 'factor' : 'news',
                            radius: item.type === 'factor' ? 20 : 15,
                            color: item.type === 'factor' ? COLORS.factor : COLORS.news,
    ...
    const handleCreateMultiverse = () => {
        ...
        if (!name) return;
        setMultiverses([...multiverses, { id, name }]);
        setActiveMultiverse(id);
    };

    const handleDeleteSelected = () => {
        if (!selectedNode) return;
        if (selectedNode.id === 'core') return; // Cannot delete core
    ...
    return (
        <div className="w-full h-full relative bg-slate-950 overflow-hidden flex flex-col">
            ...
            <div className="absolute top-4 left-4 z-50 pointer-events-auto flex flex-col gap-3">
                <div className="bg-slate-900/80 backdrop-blur border border-slate-700 p-2.5 rounded-xl flex items-center gap-4 shadow-xl">
                    <div className="flex items-center gap-2 pr-4 border-r border-slate-700">
                        <Folder className="text-cyan-400" size={16} />
                        <select
                            value={activeMultiverse}
                            onChange={(e) => setActiveMultiverse(e.target.value)}
                ...
                {selectedNode && (
                    <div className="bg-slate-900/80 backdrop-blur border border-slate-700 p-3 rounded-xl shadow-xl w-64 animate-slide-up">
                        <div className="text-[10px] font-bold tracking-widest uppercase mb-1 flex items-center justify-between" style={{ color: selectedNode.color }}>
                            ...
                            {selectedNode.id !== 'core' && (
                                <button onClick={handleDeleteSelected} className="text-slate-500 hover:text-rose-400 p-1"><Trash2 size={12} /></button>
                            )}
                        </div>
                ...
                {selectedLink && (
                    <div className="bg-slate-900/90 backdrop-blur border border-slate-700 p-4 rounded-xl shadow-2xl w-80 animate-slide-up relative">
                        ...
                        <div className="mt-4 flex justify-end">
                            <button onClick={handleDeleteSelectedLink} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-rose-400 hover:bg-slate-800 rounded transition-colors">
